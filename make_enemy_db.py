...
def execute_sql_on_csv(csv_file_path, json_file_path):
    ...
    with open(csv_file_path, newline="") as csvfile:
        ...
        for row in csvreader:
            ...
            wasd = row.get("attack_bonus").split()
            attack_bonus = sum(list(map(lambda x: int(x.strip(",+")), wasd))) / max(
                1, len(wasd)  # unseen servant has 0 attacks
            )
            ...
            try:
                ...
                ac = fortitude = will = spell_dc = spell_attack_bonus = 0

            cursor.execute(
                """INSERT INTO foo (level, hp, ac, fortitude, will, attack_bonus, spell_dc, spell_attack_bonus)
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    level,
                    hp,
